...
# -------------------------------
# Movement Parameters & Position Tracking
# -------------------------------
...
# -------------------------------
# Robot Control & MPU6050–based Turning
# -------------------------------
LMPins = (8, 11)
RMPins = (10, 18)
PWMPins = (7, 9)
...
current_angle = 0.0  # Global heading estimate

def gyro_cal(samples=150):
    """Calibrates the MPU6050 gyroscope by averaging multiple samples."""
    gyro_z = 0.0
...
def turn_to_angle_mpu(target_angle, current_angle):
    """PID–controlled turn using MPU6050 data.
       Returns the updated global angle after turning."""
    bias_z = gyro_cal()
    prev_time = time.time()
    init_angle = 0.0
    Kp = 0.6
...
def controlled_turn(target_heading):
    """Uses the MPU6050-based turn function to rotate the robot to target_heading."""
    global current_angle
    Robot.stop()
    current_angle = turn_to_angle_mpu(target_heading, current_angle)
    return True
...
# -------------------------------
# Navigation Core & Obstacle Avoidance
# -------------------------------
def avoid_obstacle(keyboard_obj, depth_frame, img_width):
    Robot.stop()
    orig_heading = current_angle
...
# Main Program
# -------------------------------
# Example waypoints (the first point is assumed to be the starting point)
